...
const Cart = () => {
  ...
  const handleDelete = (id: number) => {
    const updatedProduts = products.filter((item) => item._id !== id);
    setProducts(updatedProduts);
    localStorage.setItem("cart", JSON.stringify(updatedProduts));
    calculateSubTotal(updatedProduts);
  };

  const handleQuantity = (id: number, type: string) => {
    const updatedProduct = products.map((item: any) => {
      if (item._id === id) {
        console.log(item, "item");
        const newQuantity =
          type === "remove" ? item.quantity - 1 : item.quantity + 1;
        const quantity =
          item.quantity === 0 && type === "remove" ? 0 : newQuantity;
    ...
    calculateSubTotal(updatedProduct);
  };
                    ...
                    <div className="flex flex-col gap-4">
                      ...
                      <div className="mt-auto flex items-center gap-3">
                        <button
                          ...
                          onClick={() => handleQuantity(item._id, "remove")}
                        >
                          <svg
                        ...
                        <button
                          type="button"
                          className="flex items-center justify-center w-5 h-5 bg-gray-400 outline-none rounded-full"
                          onClick={() => handleQuantity(item._id, "add")}
                        >
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            className="w-2 fill-white"
          ...
          <div className="bg-white rounded-md px-4 py-6 h-max shadow-[0_2px_12px_-3px_rgba(6,81,237,0.3)]">
            <ul className="text-gray-800 space-y-4">
              <li className="flex flex-wrap gap-4 text-sm">
                Subtotal{" "}
                <span className="ml-auto font-bold">{subTotal} Rs.</span>
              </li>
              <li className="flex flex-wrap gap-4 text-sm">
                Shipping <span className="ml-auto font-bold">50.00 Rs.</span>
              </li>
              <li className="flex flex-wrap gap-4 text-sm">
                Tax <span className="ml-auto font-bold">20.00 Rs.</span>
              </li>
              <hr className="border-gray-300" />
              <li className="flex flex-wrap gap-4 text-sm font-bold">
                Total <span className="ml-auto">{subTotal + 50 + 20} Rs.</span>
              </li>
            </ul>
